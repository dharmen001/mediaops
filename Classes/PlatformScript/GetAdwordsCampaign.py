...
REPORT_DOWNLOAD_DIRECTORY = c.section_value[35]
...
def _DownloadReport(process_id, report_download_directory, customer_id,
    ...
    Returns:
      ...
      context.
    """
    report_downloader = (googleads.adwords.AdWordsClient.LoadFromStorage(c.section_value[36])
                         .GetReportDownloader())
...
if __name__ == '__main__':
    ...
    adwords_client = googleads.adwords.AdWordsClient.LoadFromStorage(c.section_value[36])
    mcc_id = pd.read_csv(c.section_value[34] + 'Client_id_mcc.csv')
    for id_new in mcc_id.iloc[:,  0]:
        print('\tRetrieving for {} MCC reports:'.format(id_new))
        adwords_client.SetClientCustomerId('{}'.format(id_new))
