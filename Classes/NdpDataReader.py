...
class NdpReader(NdpDataFile.NdpData):
    ...
    def uk_publisher_data(self):
        ...
        logger.info("Done Reading:-  UK Publisher files at " +
                    str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
        ...
        for g in files_xlsx:
            print('Start Reading filename: -' + g)
            instruction_data = pd.read_excel(self.section_value[14] + g, 'Instructions', skiprows=15)
